...
ticks <- Reduce(intersect, list(r$tick, scope1$tick, scope2$tick))
...
df <- df[which(rowSums(is.na(df[,4:5])) < 3), ]
...
df <- df[!is.infinite(df$log_scope1) & !is.infinite(df$log_scope2), ]
...
mdl <- plm(r ~ log_scope1 + log_scope2, data = panel_df, model = "within")
